...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Re-run PHASE 3.4 on preserved papers")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Reuse the latest PHASE 2.3 run instead of extracting again",
    )
    parser.add_argument("--fresh-phase2", action="store_true")
    return parser.parse_args()


def latest_phase2_run(session, paper_id: int) -> ExtractionRun | None:
    return session.scalar(
        select(ExtractionRun)
        .where(
            ExtractionRun.paper_id == paper_id,
            ExtractionRun.run_type == "case_timeline",
            ExtractionRun.schema_version == ExtractionPipeline.SCHEMA_VERSION,
        )
...
def main() -> None:
    ...
    if args.paper_id is not None:
        ...
        if not targets:
            raise SystemExit(f"Paper {args.paper_id} is not in the PHASE 3.4 set")

    with SessionLocal() as session:
        for _repeat in range(args.repeat):
            for paper_id, label in targets:
                if args.fresh_phase2:
                    print(f"=== PHASE 2 extract paper={paper_id} ({label}) ===", flush=True)
                    case_run = case_pipeline.run(session, paper_id)
                else:
                    case_run = latest_phase2_run(session, paper_id)
                    if case_run is None:
                        print(f"=== PHASE 2 extract paper={paper_id} ({label}) ===", flush=True)
                        case_run = case_pipeline.run(session, paper_id)
                    else:
                        print(
                            f"=== reuse PHASE 2 run={case_run.id} paper={paper_id} ({label}) ===",
                            flush=True,
                        )
                print(
                ...
                for case in cases:
                    ...
                    print(
                        ...
                        f"recovered={metrics.get('recovered_from_recall', 0)} "
                        f"reconcile={metrics.get('reconciliation_required', 0)}",
                        flush=True,
                    )
